...
export class ZapClient {
   ...
   * Progress is 0-100
   */
  async waitForScan(scanId: string, maxWaitMs: number = 600000): Promise<void> {
    const startTime = Date.now();
    ...
    while (Date.now() - startTime < maxWaitMs) {
      try {
        ...
        // استمر في المحاولة بدلاً من إلقاء خطأ
        await new Promise((resolve) => setTimeout(resolve, pollInterval));
      }
    }
   ...
   */
  async performScan(targetUrl: string): Promise<ZapScanResult> {
